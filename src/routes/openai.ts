...
const validateBasicMediaInputs = (
  orderedMedia: Array<{ type: RefType; file: Express.Multer.File; index: number }>
): ValidationDetail[] => {
  const details: ValidationDetail[] = [];
  const totalBytes = orderedMedia.reduce((sum, item) => sum + item.file.size, 0);
  ...
  for (const item of orderedMedia) {
    ...
    if (item.type === 'image') {
      if (!IMAGE_EXTS.has(ext)) details.push({ field: 'image', file: f.originalname, message: 'unsupported image format; use jpeg/png/webp/bmp/tiff/gif' });
      if (f.size > IMAGE_MAX_BYTES) details.push({ field: 'image', file: f.originalname, message: `image size ${(f.size / MB).toFixed(2)}MB exceeds the 30MB limit` });
    } else if (item.type === 'video') {
      if (!VIDEO_EXTS.has(ext)) details.push({ field: 'video', file: f.originalname, message: 'unsupported video format; use mp4/mov' });
      if (f.size > VIDEO_MAX_BYTES) details.push({ field: 'video', file: f.originalname, message: `video size ${(f.size / MB).toFixed(2)}MB exceeds the 50MB limit` });
    } else {
      if (!AUDIO_EXTS.has(ext)) details.push({ field: 'audio', file: f.originalname, message: 'unsupported audio format; use wav/mp3' });
      if (f.size > AUDIO_MAX_BYTES) details.push({ field: 'audio', file: f.originalname, message: `audio size ${(f.size / MB).toFixed(2)}MB exceeds the 15MB limit` });
    }
  }
  return details;
};
...
const saveResponseInputFile = async (input: string, type: RefType): Promise<Express.Multer.File> => {
  ...
  if (dataUrl) {
    ...
    if (buffer.length > maxBytes) throw httpError(400, `${type} input exceeds ${(maxBytes / MB).toFixed(0)}MB`);
  ...
  if (text.startsWith('http://') || text.startsWith('https://')) {
    ...
    let response;
    try {
      response = await downloadRemoteInput(text, { maxBytes, timeoutMs: 30000 });
    } catch (error: any) {
      throw httpError(400, error.message || 'remote URL is not allowed');
    }
    const mime = response.contentType;
    const ext = getMimeExt(mime) || path.extname(response.url.pathname) || '.bin';
    const file = makeTempFileFromBuffer(response.buffer, ext, path.basename(response.url.pathname) || `remote-input${ext}`);
    file.mimetype = mime || inferMimeType(ext, type);
    return file;
  }
...
const markSubmitProgress = (
  req: Request,
  phase: Parameters<typeof updateSubmitProgress>[1],
  patch: Parameters<typeof updateSubmitProgress>[2] = {}
) => {
  const traceId = getSubmitTraceId(req);
  if (!traceId) return;
  updateSubmitProgress(traceId, phase, {
    method: req.method,
...
const mapTaskStatusToOpenAIVideoStatus = (status: string, task?: any) => {
  if (status === 'SUCCESS') return 'completed';
  if (status === 'FAILED') return 'failed';
  const queueStatus = String(task?.queueStatus || '').toLowerCase();
  const progress = Number(task?.progressPercent);
  if (progress > 0 || /process|running|generat/.test(queueStatus)) return 'in_progress';
  return 'queued';
};

const taskProgressPercent = (task: any, doneStatus: string) => {
  const progress = Number(task?.progressPercent);
  if (Number.isFinite(progress)) return Math.max(0, Math.min(100, Math.round(progress)));
  return doneStatus === 'completed' ? 100 : 0;
};

const compactQueueInfo = (task: any, fallbackStatus?: string) => {
  const queue = {
...
const responsePollUrl = (taskId: string) => `/v1/responses/resp_${taskId}`;
const videoPollUrl = (taskId: string) => `/v1/videos/video_${taskId}`;
...
const dispatchImageTask = async (
  ...
  try {
    ...
    const validationDetails = validateBasicMediaInputs(files.map((file, index) => ({ type: 'image' as RefType, file, index })));
...
router.get('/models', apiKeyAuth, (req: Request, res: Response) => {
...
router.get('/submit-progress/:id', apiKeyAuth, (req: Request, res: Response) => {
  const progress = getSubmitProgress(String(req.params.id || ''));
  if (!progress) return res.status(404).json({ error: { message: 'Submit progress not found' } });
