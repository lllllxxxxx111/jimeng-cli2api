...
const assertInside = (root, target, message) => {
    ...
    if (resolvedTarget === resolvedRoot || resolvedTarget.startsWith(resolvedRoot + path_1.default.sep)) {
        return resolvedTarget;
    }
    throw new Error(message);
};
const safeAccountDirName = (name) => {
...
router.post('/sys/password', adminAuth_1.adminAuth, async (req, res) => {
    ...
    if (match) {
        ...
        config.token = 'admin_token_' + (0, crypto_1.randomBytes)(32).toString('hex'); // force relogin
        fs_1.default.writeFileSync(configPath, JSON.stringify(config));
...
const updateAccountFailure = async (accountId, error) => {
    return prisma.jimengAccount.update({
        where: { id: accountId },
        data: { status: (0, accountFailure_1.accountStatusForFailure)(error), lastChecked: new Date() },
    });
};
const accountFailureMessage = (error) => {
    const failure = (0, accountFailure_1.classifyAccountFailure)(error);
...
router.post('/accounts/login', async (req, res) => {
    ...
    if (displayName.length > 50)
        return res.status(400).json({ error: '账号名称不能超过 50 个字符' });
    try {
        const homeDir = assertInside(ACCOUNTS_ROOT, path_1.default.resolve(ACCOUNTS_ROOT, `${safeAccountDirName(displayName)}_${Date.now()}`), 'account homeDir is outside the allowed data/accounts folder');
...
router.post('/accounts/:id/headless-login', async (req, res) => {
    try {
        const account = await prisma.jimengAccount.findUnique({ where: { id: req.params.id } });
        if (!account)
            return res.status(404).json({ error: "账号不存在" });
        ...
        if (oauthResult.alreadyLoggedIn) {
            ...
            try {
                ...
                return res.json({
                    ...
                    alreadyLoggedIn: true,
                    mode: 'login',
                    rawOutput: oauthResult.rawOutput,
                ...
                return res.status((0, accountFailure_1.classifyAccountFailure)(error.message || error)?.status === 'NO_VIP' ? 403 : 500).json({
...
router.post('/accounts/:id/checklogin', async (req, res) => {
    try {
        ...
        try {
            ...
            return res.status((0, accountFailure_1.classifyAccountFailure)(checkOutput)?.status === 'NO_VIP' ? 403 : 500).json({
...
router.post('/accounts/:id/import-current-login', async (req, res) => {
    try {
        ...
        const account = await prisma.jimengAccount.findUnique({ where: { id: req.params.id } });
        const failure = (0, accountFailure_1.classifyAccountFailure)(error.message || error);
...
router.delete('/accounts/:id', async (req, res) => {
    try {
        ...
        if (!account)
            return res.status(404).json({ error: '账号不存在' });
        const absoluteHome = assertInside(ACCOUNTS_ROOT, path_1.default.resolve(account.homeDir), 'account homeDir is outside the allowed data/accounts folder');
        // 先删 DB 记录
        await prisma.jimengAccount.delete({ where: { id: account.id } });
...
router.post('/accounts/:id/check', async (req, res) => {
    try {
        ...
        const { stdout } = await (0, cliRunner_1.runJimengCommand)('dreamina user_credit', account.homeDir, false, ACCOUNT_CHECK_TIMEOUT_MS);
        const newBalance = parseCreditBalance(stdout, account.creditBalance);
        (0, cliRunner_1.saveCredentialBackup)(path_1.default.resolve(account.homeDir));
        await (0, cliRunner_1.saveRegToken)(path_1.default.resolve(account.homeDir));
...
const taskToolWhere = (tool) => {
    if (tool === 'image_generation')
        return { OR: [{ toolType: 'image_generation' }, { type: { in: ['text2image', 'image2image'] } }] };
    ...
    if (tool === 'image_upscale')
        return { OR: [{ toolType: 'image_upscale' }, { type: 'image_upscale' }] };
    return { toolType: tool };
};
router.get('/tasks', async (req, res) => {
    try {
        const { status, accountId, prompt, error, type, model, tool, page = '1', limit = '20' } = req.query;
        const where = {};
        const and = [];
        if (status)
            where.status = status;
        ...
        if (error)
            and.push({ OR: [{ errorMsg: { contains: error } }, { pollErrorMsg: { contains: error } }] });
        if (type)
            where.type = type;
        if (model)
            where.model = model;
        if (tool)
            and.push(taskToolWhere(tool));
        if (and.length)
            where.AND = and;
