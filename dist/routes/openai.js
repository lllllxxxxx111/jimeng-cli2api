...
const dispatchImageTask = async (req, body, files, source = {}) => {
    try {
...
const dispatchUpscaleTask = async (req, body, file, source = {}) => {
    try {
        if (!file)
...
const dispatchVideoTask = async (req, body, filesMap, source = {}) => {
    const allFiles = [
        ...(filesMap.image || []),
        ...(filesMap.audio || []),
        ...(filesMap.video || []),
...
router.post('/images/upscale', apiKeyAuth, upload.single('image'), async (req, res) => {
    try {
        ...
        const result = await dispatchUpscaleTask(req, req.body || {}, req.file, { entrypoint: 'images_upscale', toolType: 'image_upscale' });
        return res.json({
            id: result.id,
            status: result.status,
            submit_id: result.submit_id,
...
router.get('/tasks/:id', apiKeyAuth, async (req, res) => {
    try {
        ...
        const response = { id: task.id, status: "processing", task_type: task.type, entrypoint: task.entrypoint, tool_type: task.toolType, progress, queue };
        if (task.pollErrorMsg) {
